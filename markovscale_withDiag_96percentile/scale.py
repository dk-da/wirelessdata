...
for sample1 in sampleList:
	...
	for sample2 in sampleList[sampleList.index(sample1):]:
		...
		if (sample1 == sample2):
			...
			GcorList = crawl(STARTTIME, ENDTIME, timescale, macDic[sample1.hashedmac], macDic[sample2.hashedmac])
			if len(GcorList) == 0:continue
			if (sample1.owner == sample2.owner):
				line = "%d %f %f %d %s %s %s" % (1, numpy.average(GcorList), numpy.median(GcorList), len(GcorList), sample1.owner+"."+sample1.device+"."+sample2.owner+"."+sample2.device, sample1.hashedmac, sample2.hashedmac)
				print >> outf, line
			else:
				line = "%d %f %f %d %s %s %s" % (0, numpy.average(GcorList), numpy.median(GcorList), len(GcorList), sample1.owner+"."+sample1.device+"."+sample2.owner+"."+sample2.device, sample1.hashedmac, sample2.hashedmac)
				print >> outf, line
		print "1"
